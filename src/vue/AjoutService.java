    ...
    private void ajouterActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ajouterActionPerformed
        ...
        if (numServ.getText().equals("")) {
            ...
            try {
                ...
                // Vérifier si le numServ existe déjà
                String requeteNumServExistence = "SELECT COUNT(*) FROM service WHERE numServ = ?";
                pst = conn.prepareStatement(requeteNumServExistence);
                pst.setString(1, numServValue);
                ResultSet rsNumServExistence = pst.executeQuery();
                rsNumServExistence.next();
                int countNumServExistence = rsNumServExistence.getInt(1);

                if (countNumServExistence > 0) {
                    JOptionPane.showMessageDialog(null, "Le numéro de service existe déjà");
                } else {
                    // Vérifier si le service existe déjà
                    String requeteServiceExistence = "SELECT COUNT(*) FROM service WHERE service = ?";
                    pst = conn.prepareStatement(requeteServiceExistence);
                    pst.setString(1, service);
                    ResultSet rsServiceExistence = pst.executeQuery();
                    rsServiceExistence.next();
                    int countServiceExistence = rsServiceExistence.getInt(1);

                    if (countServiceExistence > 0) {
                        JOptionPane.showMessageDialog(null, "Le service existe déjà");
                    } else {
                        // Insérer le nouveau service
                        String requeteInsertion = "INSERT INTO service (numServ, service, prix) VALUES (?, ?, ?)";
